...
import {
  TOTAL_NET_WORTH, NET_WORTH_HISTORY, GOALS, MILESTONES,
} from "@/lib/mock-data";
...
function getProjections() {
  return {
    "6mo":  projectNetWorth(NET_WORTH_HISTORY, 26),
    "1yr":  projectNetWorth(NET_WORTH_HISTORY, 52),
    "3yr":  projectNetWorth(NET_WORTH_HISTORY, 156),
  };
}
...
function GoalBar({ goal, current }) {
  ...
  // Weeks to hit goal based on avg weekly growth
  const recent = NET_WORTH_HISTORY.slice(-8);
  ...
  const monthsLeft = weeksLeft ? Math.round(weeksLeft / 4.33) : null;
    ...
    <div className="card lift fade-up" style={{
        ...
        <div className="xp-bar-track" style={{ height: 7 }}>
          <div
            ...
            style={{
              ...
              width: `${pct}%`,
              background: achieved
                ? "linear-gradient(90deg, #1A7A38, var(--positive))"
                : undefined,
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--ink)", marginTop: 2 }}>
            {fmt(current)}
          </div>
        </div>
        {!achieved && remaining > 0 && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--negative)", marginTop: 2 }}>
              {fmt(remaining)}
            </div>
          </div>
        )}
        <div style={{ textAlign: "right" }}>
          <div className="label">{achieved ? "Completed" : "Est. arrival"}</div>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: achieved ? "var(--positive)" : "var(--ink2)", marginTop: 2 }}>
            {achieved
              ? goal.achievedAt || "Done"
              : monthsLeft !== null
                ? monthsLeft < 1 ? "This month" : `~${monthsLeft}mo`
                : "—"
            }
          </div>
        </div>
...
function ProjectionCard({ label, value, current, delay }) {
  ...
  return (
    <div className={`card lift fade-up fade-up-${delay}`} style={{
...
export default function MilestonesPage() {
  ...
  const current = TOTAL_NET_WORTH;

  // Next milestone (nearest target above current)
  const nextMilestone = [100000, 150000, 200000, 250000, 500000]
    .find(m => m > current) || 0;
  const nextPct = nextMilestone ? ((current / nextMilestone) * 100) : 100;
      ...
      <main className="page" style={{ paddingTop: 16, display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div className="fade-up">
          ...
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--ink2)", letterSpacing: "0.08em" }}>
            Goals, projections & achievements
          </div>
        </div>
        ...
        <div className="card card-glow fade-up" style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 14 }}>
            <div>
              <div style={{ fontFamily: "var(--font-display)", fontSize: 32, color: "var(--ink)", lineHeight: 1, marginBottom: 4 }}>
                <CountUp target={current} duration={1000} />
              </div>
          ...
          <div className="xp-bar-track" style={{ height: 8 }}>
            <div
              className="xp-bar-fill"
              style={{ "--xp-width": `${nextPct}%`, width: `${nextPct}%`, height: "100%" }}
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {GOALS.map((goal, i) => (
              <GoalBar key={goal.id} goal={goal} current={current} />
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="card" style={{ padding: "4px 18px" }}>
            {[...MILESTONES]
              ...
              .map((m, i) => (
                <MilestoneBadge key={m.amount} milestone={m} index={i} />
              ))
            }
            {MILESTONES.length === 0 && (
              <div style={{
